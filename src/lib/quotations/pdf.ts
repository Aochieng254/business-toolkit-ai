...
export async function generateQuotationPDF(
  ...
  let hy = y + 36;
  hy = drawWrapped(`#${quotation.quotation_number}`, pageW - margin - halfW, hy, halfW, 14, "right");
  hy = drawWrapped(`Date: ${quotation.quotation_date}`, pageW - margin - halfW, hy, halfW, 14, "right");
  if (quotation.valid_until)
    hy = drawWrapped(`Valid until: ${quotation.valid_until}`, pageW - margin - halfW, hy, halfW, 14, "right");
  if (quotation.reference_number)
    hy = drawWrapped(`Ref: ${quotation.reference_number}`, pageW - margin - halfW, hy, halfW, 14, "right");
  ...
  if (customer) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(10);
    rightBottom = drawWrapped("Prepared For", rx, rightBottom, halfW, 13, "right");
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    for (const line of [
      customer.name,
  ...
  if (quotation.sales_rep) {
    doc.setFont("helvetica", "normal");
    doc.setFontSize(9);
    y = drawWrapped(`Sales rep: ${quotation.sales_rep}`, margin, y, halfW, 12) + 6;
  }
  ...
  if (quotation.notes) drawSection("Notes", quotation.notes);
  if (quotation.terms) drawSection("Terms & Conditions", quotation.terms);
